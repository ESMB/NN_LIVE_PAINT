...
filename_contains="results.csv"

# Folders to analyse:
root_path=r"//Users/Mathew/Documents/Current analysis/Haresh_LIVE_PAINT/"
pathList=[]



# pathList.append(r"/Users/Mathew/Documents/Current analysis/Synaptosomes_Peptide_PAINT/500nM/20")
pathList.append(r"/Users/Mathew/Documents/Current analysis/Haresh_LIVE_PAINT/")
...
def NN_cluster_all():
    ...
    for num in range(0,maximum):
        ...
        scale_cluster_ycoord=(cluster_ycoord)
        scale_cluster_xcoord=(cluster_xcoord)
        cluster_coords=coords[np.where(clusters==num)]
    # Need to find distances between all points
    ...
    # Now make a plot
        ...
        for i in range(0,100):
            for row in num_points:
                if row==i:
                    xcoord_plot.append(8*scale_cluster_xcoord[row])
                    ycoord_plot.append(8*scale_cluster_ycoord[row])
                    points_plot.append(i)
            
        # for i in range(0,100):
        #     x_coord_all=xcoord_plot[np.where(points_plot==i)]
        #     y_coord_all=ycoord_plot[np.where(points_plot==i)]
    ...
    return xcoord_plot,ycoord_plot,points_plot
                ...
                for name in files:
                        if filename_contains in name:
                                ...
                                if ".tif" not in name:
                                    ...
                                    index_names = loc_data[loc_data['uncertainty [nm]']>prec_thresh].index
                                    ...
                                    precsx= np.array(data_df['uncertainty [nm]'])
                                    precsy= np.array(data_df['uncertainty [nm]'])
                                    xcoords=data_df['x [nm]']/Pixel_size
                                    ycoords=data_df['y [nm]']/Pixel_size
                                    
                                    xcoords= xcoords.to_numpy()
                                    ycoords= ycoords.to_numpy()
                                    ...
                                    if to_cluster==1:
                                        ...
                                        if len(cluster_contents)>0:
                                            ...
                                            xpoints,ypoints,values= NN_cluster_all()
                                            ...
                                            scatter = ax.scatter(xpoints, ypoints, c=values, cmap='jet')
                                            ...
                                            # Add a color bar
                                            cbar = fig.colorbar(scatter, ax=ax, label='Local Density')
                                            cbar.set_label('Local Density', fontsize=40)
                                            cbar.ax.tick_params(labelsize=40)
                                            # Show the plot
                                            plt.savefig(path+"_Map.pdf") 
                                            plt.show()
